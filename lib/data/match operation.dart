...
class MatchOperation {
  ...
  Future<List<Match>?> getAllMatch() async {
    ...
    List<Match>? matches = allRows?.map((matches) => Match.fromMap(matches)).toList();
    return matches;
  }

  Future<List?> searchAllMatchById(int id) async {
    final db = await dbProvider.database;
    List<Map<String, dynamic>>? allRows = await db?.query('match', where: 'matchId LIKE ?', whereArgs: ['%$id%']);
    List? matches =
    allRows?.map((match) => Match.fromMap(match)).toList();
    return matches;
  }
}
